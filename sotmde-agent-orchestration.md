...
## Pre-Flight Setup (human operator — do this before Agent-0)

Agent-0's exit gate requires `node .` to start and `localhost:4000` to load. Three
things must be true before that can happen; none of them are handled by any agent.

### 1. Start infrastructure services

MongoDB and Redis are not installed natively. The `config/default.json5` hostnames
(`mongo`, `redis`) are Docker Compose service names that only resolve inside Docker's
internal network — `node .` run natively will fail to connect without this step.

Docker is available. Bring up only the infrastructure services (not the app itself):
...
```bash
docker compose ps          # mongo and redis should show "running"
```

### 2. Create `config/local.json5`

`config/default.json5` uses Docker service hostnames. Override them for native
development with a local config file (this file is gitignored and should never be
committed):

```json5
// config/local.json5
{
  mongo: 'mongodb://localhost:27017/ashteki',
  redisUrl: 'redis://localhost:6379/'
}
```

> Agent-0 will rename the database from `ashteki` to `sentinels` in Step 0.1. Update
> this file to `mongodb://localhost:27017/sentinels` once that step is complete.

### 3. Install dependencies

`node_modules` is empty — `npm install` has not been run on this machine yet:
...
- [ ] `docker compose ps` shows `mongo` and `redis` in running state
- [ ] `config/local.json5` exists with `localhost` URLs
- [ ] `npm install` completed without errors
- [ ] `node .` starts and `localhost:4000` loads (smoke-check before handing to Agent-0)

Once all four items pass, hand off to Agent-0.
...
> of Ashteki at `/home/sjzhu/repos/heroteki`. The goal is to rename all project
...
> The repo is at `/home/sjzhu/repos/heroteki`. Phase 0 (renaming) is already done.
...
2. Stub all rules-engine files from Step 1.2: `cardaction.js`, `abilitycontext.js`,
   `abilitytarget.js`, `costs/` (dir), `effects/` (dir), `gameactions/` (dir),
   `promptinterrupter.js`, `triggeredabilitycontext.js`; delete directories
   `gamesteps/`, `solo/`, `Clocks/`; stub `EffectEngine.js`, `GamePipeline.js`,
   `CardVisibility.js`, `chatcommands.js`, `loader.js`, `deck.js`; strip but
   keep `gamechat.js`
...
> The repo is at `/home/sjzhu/repos/heroteki`. Phases 0–1 are complete (Ashes code
...
> The repo is at `/home/sjzhu/repos/heroteki`. Steps 2.1–2.5 are complete (schemas,
...
> The repo is at `/home/sjzhu/repos/heroteki`. Phases 0–2 are complete (data model
...
> The repo is at `/home/sjzhu/repos/heroteki`. Phase 3 (server game model) is
...
> The repo is at `/home/sjzhu/repos/heroteki`. Read the plan fully before starting.
...
> The repo is at `/home/sjzhu/repos/heroteki`. Phases 1–7 are complete.
...
> The repo is at `/home/sjzhu/repos/heroteki`. Phases 1–8.5 are complete.
> Read the plan fully before starting.
...
> The repo is at `/home/sjzhu/repos/heroteki`. All prior phases are complete.
> Read the plan fully before starting.
