...
const CardLog = ({ items, onMouseOut, onMouseOver }) => {
    ...
    const renderItem = (item, last = false) => {
        ...
        const itemClass = classNames('vertical mb-2', last ? 'last-card' : 'target-card');
    ...
    const cardPics = items.length > 1 ? items.slice(0, logLength).map((c) => renderItem(c)) : null;
    const firstCard = items.length ? renderItem(items[logLength], true) : null;
